...
export default scene => {
  ...
  //======ANIMATIONS=====//
  const cubeFooterAnim = new TimelineMax();
        cubeFooterAnim.from(pivot.rotation, 1, { z:(40 * deg), x: (-10 * deg) }).pause();
        cubeFooterAnim.from(cubeGroup.rotation, 1, { y:(245 * deg) }, '-=1').pause();
        cubeFooterAnim.from(cubeGroup.position, 1, { y: 0.5, z: -5 }, '-=1').pause();
        window.cubeFooterAnim = cubeFooterAnim;
  //======ANIMATIONS=====//
  ...
  const getCube = (modelName) => {
    ...
    return new Promise((resolve) => {
      loaderMtl.load('/static/obj/cube_footer.mtl', materials => {

        materials.preload();
        loaderObj.setMaterials(materials);
        loaderObj.load('/static/obj/cube_footer.obj', (obj) => {
